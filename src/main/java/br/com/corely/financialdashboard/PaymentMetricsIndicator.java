...
public class PaymentMetricsIndicator {
    ...
    private List<PaymentDayResponse> calculateByDay(LocalDate monthStart, LocalDate monthEnd) {
        List<Object[]> data = paymentRepository.paymentsPerDay(monthStart, monthEnd);
    ...
    private List<PaymentMonthResponse> calculateByMonth() {
        ...
        String startReferenceMonth = now.minusMonths(11).format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM"));
        List<Object[]> data = paymentRepository.paymentsPerMonth(startReferenceMonth);
