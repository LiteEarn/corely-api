...
class TenantIsolationTest {
    ...
    void findById_shouldReturn200_whenSessionBelongsToCurrentTenant() throws Exception {
        mockMvc.perform(get("/class-sessions/{id}", sessionA.getId()))
                .andExpect(status().isOk());
    }

    @Test
    void findById_shouldReturn404_whenSessionBelongsToOtherTenant() throws Exception {
        mockMvc.perform(get("/class-sessions/{id}", sessionB.getId()))
    ...
    void cancel_shouldReturn404_whenSessionBelongsToOtherTenant() throws Exception {
        ...
        request.setCancelReason(CancelReason.OTHER);
        request.setCancelDescription("Hacked cancel");

        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                        .patch("/class-sessions/{id}/cancel", sessionB.getId())
