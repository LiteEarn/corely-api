...
@ActiveProfiles("test")
@Transactional
class ContractRenewalIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        studio = studioRepository.save(createStudio("Renewal Studio"));
        authenticateAs(studio, UserRole.OWNER);

        student = studentRepository.save(createStudent(studio, "John Renewal"));

        plan = planRepository.save(createPlan("Standard Plan", BigDecimal.valueOf(199), 30, true));
        ...
        var response = contractApplicationService.enroll(spRequest);
        studentPlanId = response.getId();
    }
    ...
    @Test
    void process_shouldRenewEligibleContract() {
        expireContract();

        var result = contractRenewalService.process(LocalDate.of(2026, 7, 14));
        ...
        var studentPlan = studentPlanRepository.findById(studentPlanId).orElseThrow();
        assertThat(studentPlan.getEndDate()).isEqualTo(LocalDate.of(2026, 7, 30));
        assertThat(studentPlan.getStatus()).isEqualTo(StudentPlanStatus.ACTIVE);
    }

    @Test
    void process_shouldCreateNewContractSnapshot() {
        expireContract();

        var oldSnapshots = contractSnapshotRepository.findAll();
        var oldCount = oldSnapshots.size();

        contractRenewalService.process(LocalDate.of(2026, 7, 14));

        var newSnapshots = contractSnapshotRepository.findAll();
        assertThat(newSnapshots.size()).isEqualTo(oldCount + 1);

        var studentPlan = studentPlanRepository.findById(studentPlanId).orElseThrow();
        assertThat(studentPlan.getContractSnapshot().getId()).isNotEqualTo(oldSnapshots.get(0).getId());
    }

    @Test
    void process_shouldCreateBillingScheduleWhenMissing() {
        expireContract();

        var existingSchedule = billingScheduleRepository.findByStudentPlanId(studentPlanId);
        existingSchedule.ifPresent(s -> {
            s.setActive(false);
            billingScheduleRepository.save(s);
        });

        contractRenewalService.process(LocalDate.of(2026, 7, 14));

        var schedule = billingScheduleRepository.findByStudentPlanId(studentPlanId);
        assertThat(schedule).isPresent();
        assertThat(schedule.get().getActive()).isTrue();
    }
    ...
    void process_shouldSkipWhenOverdueInvoicesExist() {
        ...
        var studentPlan = studentPlanRepository.findById(studentPlanId).orElseThrow();
        assertThat(studentPlan.getEndDate()).isEqualTo(LocalDate.of(2026, 6, 30));
    }
    ...
    @Test
    void process_shouldNotProcessNonExpiringContracts() {
        var result = contractRenewalService.process(LocalDate.of(2026, 6, 15));
    ...
    @Test
    void process_shouldNotRenewCancelledContract() {
        var studentPlan = studentPlanRepository.findById(studentPlanId).orElseThrow();
    ...
    @Test
    void process_shouldNotRenewFinishedContract() {
        var studentPlan = studentPlanRepository.findById(studentPlanId).orElseThrow();
    ...
    @Test
    void process_shouldNotRenewSuspendedContract() {
        var studentPlan = studentPlanRepository.findById(studentPlanId).orElseThrow();
